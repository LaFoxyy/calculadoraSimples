...
import calcmestre.exception.ConexaoRecusadaException;
import calc.mestre.util.Util;
...
public class MestreServer implements Runnable {

	private int portaSocket;
	private int portaRMI;
	private boolean canRun = true;

	private MestreImple masterRMI;
	...
	public MestreServer(int portaSocket, int portaRMI) {
	...
	private Mensagem processar(Mensagem msg) throws IOException {
		...
		if (isBasico(operador)) {
			EscravoBasico basico = masterRMI.getBasicos().poll();
			...
			EscravoEspecial especial = masterRMI.getEspeciais().poll();
			Double a = Double.parseDouble(numero[0]);
			switch (operador) {
			case "√":
			...
			case "^":
                            Double b = Double.parseDouble(numero[1]);
                            resultado = especial.getPotenciacao(a, b);
                            break;
			}
